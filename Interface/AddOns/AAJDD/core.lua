...
local function default()
    ...
    if IsInInstance() then
        ...
        SetFont(SystemFont_LargeNamePlate, 6.5)
        SetFont(SystemFont_NamePlate, 6.5)
        SetFont(SystemFont_LargeNamePlateFixed, 6.5)
        SetFont(SystemFont_NamePlateFixed, 6.5)
        SetFont(SystemFont_NamePlateCastBar, 6.5)
        -----------------------------------------------------
        --将友方姓名板的框架尺寸设为1，由于暴雪的CVAR只是单纯的隐藏血条，必需做这个设置，才能在堆叠模式下不挤占敌方姓名板空间
        SetCVar("NamePlateVerticalScale", 1.5) --队友血条的高度尺寸
        C_NamePlate.SetNamePlateFriendlySize(60, -25)
