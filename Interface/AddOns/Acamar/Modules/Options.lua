...
function Options.GetOptions(uiType, uiName, appName)
    ...
    for _, v in pairs(DeepCopy(addon.db.global.prelearning)) do
        pdatalist[#pdatalist + 1] = v
    end

    local sort = table.sort(pdatalist, function(a, b) return a.hourlycount > b.hourlycount end)

    local display_list = {}

    for i = 1, 500 do
        if pdatalist[i] then
            display_list[i] = pdatalist[i].name .. " [" .. (pdatalist[i].hourlycount or 0) .. "]" .. " [" .. (pdatalist[i].daillycount or 0) .. "]"
        end
    end
                ...
                main_settings = {
                    ...
                    args = {
                        ...
                        do_not_disturb = {
                            ...
                            desc = L
                            ["Enable to bypass printing of progress messages (like talkative player added into learning) in chat window."],
                            width = "normal",
                        ...
                        min_interval_same_message = {
                            ...
                            name = L["Same message"],
                            desc = L
                            ["Allow only 1 message with same content sent by same player during set interval (seconds)"],
                            width = "normal",
                            order = 3.3,
                        },
                    },
                ...
                adv_settings = {
                    ...
                    args = {
                        message_hook_switch = {
                            ...
                            desc = L
                            ["Turn on messages filtering and learning engine. If turn off, messages will not be filtered."],
                            width = "normal",
                            set = function(info, val)
                        ...
                        top500_list_select = {
                            ...
                            values = function(info) return display_list end,
                            order = 7.1,
                        },
                    },
                },
